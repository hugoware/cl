...
const demo = /^\/code_demo\/?$/.test(path)
  ? 'code_demo'
  : /^\/web_demo\/?$/.test(path)
  ? 'web_demo'
  : /^\/demo\/?$/.test(path)
...
const $state = {
  ...
  set allowSpeech(enabled) {
    ...
    localStorage.setItem(
      SPEECH_ENABLEMENT_CONFIG,
      enabled ? SPEECH_ENABLED : SPEECH_DISABLED
    );
  },
  ...
  clearProject: () => {
    // cleanup
    if ($state.lesson) $state.lesson.dispose();
  ...
  hasUnsavedFiles: () => {
    ...
    const { files } = $state;
    for (const file of files) if (file.modified) return true;
    return false;
  },
  ...
  openFile: async (path, forceRefresh = false) => {
    ...
    if (!file.isOpen || forceRefresh) await $lfs.write(file.path, file.content);
  ...
  saveFile: async (path, content) => {
    ...
    const result = await $api.request('write-file', {
      projectId,
      path,
      content,
    });
    ...
    // saving file content
    if ($state.lesson) $state.lesson.invoke('saveFile', file);
  ...
  moveItems: async (paths, target) => {
    const { projectId } = $state;
    const result = await $api.request('move-items', {
      projectId,
      paths,
      target,
    });
    ...
    // saving file content
    if ($state.lesson) $state.lesson.invoke('moveItems', renames);
  ...
  uploadFile: async (file, parent, onProgress) => {
    ...
    if ('content' in result) create.content = result.content;

    // save the record
    target.children.push(create);

    // if needed
    if ($state.lesson) $state.lesson.invoke('uploadFile', this.file);
  ...
  renameItem: async (item, name) => {
    ...
    // send the request
    const result = await $api.request('rename-item', {
      projectId,
      source,
      target,
    });
  ...
  createFolder: async (name, relativeTo) => {
    const { projectId } = $state;
    const result = await $api.request('create-folder', {
      projectId,
      name,
      relativeTo,
    });
  ...
  deleteItems: async (selection) => {
    ...
    // notify
    if ($state.lesson) $state.lesson.invoke('removeItems', items);
...
async function syncProject(children = [], parent, relativeTo) {
  ...
  for (const item of children) {
    ...
    if (!('id' in item)) {
      ...
      const { ext } = item;
      item.type = item.isFolder
        ? 'folder'
        : _.includes(CODE_FILES, ext)
        ? 'code'
        : _.includes(IMAGE_FILES, ext)
        ? 'image'
        : _.includes(MEDIA_FILES, ext)
        ? 'media'
        : 'not-supported';

      // check if this is a content file
      item.hasContent = 'content' in item;
      if (item.hasContent) await $lfs.write(item.path, item.content);
    }
    ...
    if (!item.isEmpty) await syncProject(item.children, item, item.path);
  }
}
