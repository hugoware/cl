...
export default class HomeView extends View {
  ...
  onSetAvatar = (avatar) => {
    ...
    this.ui.avatar.empty().append(icon);
  };
  ...
  setFilter = (filter, isLessonsSection) => {
    this.removeClass(
      'filter-web filter-game filter-mobile filter-code no-lessons'
    );
  ...
  setView = (view) => {
    ...
    setTimeout(() => {
      ...
      if (!isProjects) source = _.orderBy(source, ['type', 'sequence']);
      ...
      _.each(source, (data) => {
        ...
        if (previous && previous.type !== data.type) previous = null;
  ...
  onSelectProject = async (event) => {
    ...
    if (Component.within(event.target, '.locked')) return;
