...
export function requirePermission(check) {
  const { requires, required, permissions, args = [] } = check;
  ...
  if (PreventActionPopUp.instance.__activated) return;
...
export default class PreventActionPopUp extends Component {
  ...
  onActivateProject = () => {
    if ($state.lesson) $state.lesson.instance.onDeny = this.onDeny;
  };
  ...
  onPreventAction = (location, message, explain) => {
    ...
    if (_.isString(location)) this.selector = evaluateSelector(location);
    // this is some coordinates
    else if (location && _.isNumber(location.x) && _.isNumber(location.y))
      this.position = location;
    // is another component
    else if (location instanceof Component) this.component = location;
    // maybe a DOM element
