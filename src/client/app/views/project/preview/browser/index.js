...
const VIEWABLE_TYPES = ['pug', 'html', 'htm'];
...
export default class BrowserMode extends Component {
  ...
  onAutoExecuteScripts = () => {
    if (!this.hasRunScripts) this.runScripts();
  };
  ...
  onRenameItem = (item) => {
    if (this.url === item.previous.path) this.url = item.path;
  };
  ...
  onDeleteItems = (paths) => {
    ...
    for (const path of paths) {
      if (path === file.path) return this.clear();
    }

    // check if a dependency might have been removed
    for (const path of paths) {
      if (view.isDependency(path)) return this.recompile();
    }
  };
  ...
  onPreviewMessage = (err, args = {}) => {
    ...
    else if (args.navigate) this.navigate(args.navigate);
  ...
  onCloseFile = (file) => {
    if (!this.hasActiveView) return;
    if (this.activeFile.path === file.path) this.clear();
  };
  ...
  onActivateFile = async (file, viewOnly) => {
    ...
    if (!_.includes(VIEWABLE_TYPES, ext)) return;
  ...
  navigate = (url) => {
    ...
    if (
      $state.lesson &&
      $state.lesson.respondsTo('beforePreviewAreaNavigate')
    ) {
  ...
  render = (view, shouldRunScripts) => {
    ...
    if (shouldRunScripts) this.runScripts();
  ...
  runScripts = () => {
    ...
    if (this.hasRunScripts) this.render(this.view);
