...
export async function handle(request, response) {
  ...
  if (!/post/i.test(request.method)) return response.redirect('/');
  ...
  try {
    ...
    request.session.first = result.first;
    request.session.last = result.last;

    // for admins, always use the classroom so
    // lesson unlocks are visible
    const isAdmin = result.role === 'admin';
    if (isAdmin) request.session.isAdmin = true;
    ...
    err = errorMessage(err, {
      ...
      account_expired:
        'This account has expired and no longer has access to CodeLab',
