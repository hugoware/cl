...
class Config {
  ...
  /** @return {string} does not require a membership */
  get allowFreeAccounts() {
    return !!this.data.allow_free_accounts;
  }

  /** @return {string} the SSL key */
  get sslKey() {
    return this.data.ssl_key;
  }

  /** @return {string} the SSL certificate */
  get sslCert() {
    return this.data.ssl_cert;
  }

  /** @return {string} the SSL certificate authority */
  get sslCA() {
    return this.data.ssl_ca;
  }

  /** @returns {string} key for accessing chargebee */
  get chargebeeApiKey() {
    return this.data.chargebee_api_key;
  }

  /** @returns {string} key for accessing which chargebee database */
  get chargebeeApiTarget() {
    return this.data.chargebee_api_target;
  }

  /** @returns {string} key for accessing clicksend */
  get clicksendApiKey() {
    return this.data.clicksend_api_key;
  }

  /** @returns {string} username for accessing clicksend */
  get clicksendUsername() {
    return this.data.clicksend_username;
  }

  /** @returns {string} key for recording speech */
  get azureSubscriptionKey() {
    return this.data.azure_subscription_key;
  }

  /** @returns {string} the web client ID for google auth */
  get googleWebclientId() {
    return this.data.google_webclient_id;
  }

  /** @returns {string} the secret to use for sessions */
  get sessionSecret() {
    return this.data.session_secret;
  }

  /** @returns {number} the web server port number */
  get httpPort() {
    return this.data.http_port;
  }
  ...
  get databaseUsername() {
    return this.data.database_username;
  }
